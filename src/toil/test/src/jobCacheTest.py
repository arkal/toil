...
class Hidden:
    ...
    class AbstractCacheTest(ToilTest):
        ...
        def testCacheEviction(self):
            ...
            self.options.clean = 'always'
            testData = [(20, 30, 10), (20, 30, 30), (20, 30, 60)]
            for file1, file2, diskRequestMB in testData:
                if diskRequestMB > 50:
                    # This can be non int as it will never reach _probeJobReqs
                    expectedResult = 'Fail'
                else:
                    expectedResult = 50 - file1 if diskRequestMB <= file1 else 0
                try:
                    A = Job.wrapJobFn(self._writeFileToJobStore, isLocalFile=True, fileMB=file1)
                    ...
                    C = Job.wrapJobFn(self._writeFileToJobStore, isLocalFile=True, fileMB=file2)
